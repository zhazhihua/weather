...
function Home() {
  ...
  function getWeatherData() {
    axios
      .get(
        `https://v0.yiketianqi.com/api/worldchina?appid=24831698&appsecret=p8sIVJ6B&city=${city}`
      )
      .then((res) => {
  ...
  function getNowWeatherData() {
    axios
      .get(
        `https://v0.yiketianqi.com/api?unescape=1&version=v61&appid=24831698&appsecret=p8sIVJ6B&city=${city}`
      )
      .then((res) => {
    ...
    <div className={nowWeather.wea == '晴' ? 'sunny' : 'cloud'}>
      <div className="location">
        ...
        <Search
          ...
          placeholder="输入城市"
          onSearch={onSearch}
          style={{ marginLeft: '30px', width: 150, height: 15 }}
        />
      </div>
      ...
      <div className="wind">
        <span>
          {nowWeather.wea}&nbsp;&nbsp;|&nbsp;&nbsp;{nowWeather.win}
          {nowWeather.win_speed}
        </span>
      </div>
      <div className="air">
        {nowWeather.air_pm25}
        {nowWeather.air_level}
      </div>
      <p className="todayWeather">
        今天:&nbsp;&nbsp;{nowWeather.wea}&nbsp;&nbsp; {nowWeather.tem2}至
        {nowWeather.tem1}℃ &nbsp;&nbsp;{nowWeather.win}
        {nowWeather.win_speed}
      ...
      <ul className="pic">
        {hours.map((item, index) => {
          return dayforecast == 1 ? (
            <li
              className={`iconfont ${data[(item.day || {}).icon]}`}
              key={index}
            ></li>
          ) : (
            <li className={`iconfont ${data[item.icon]}`} key={index}></li>
          )
        })}
      </ul>
          ...
          {month.map((item, index) => {
            ...
            return (
              <li key={index}>
                ...
                <p>
                  <i className={`iconfont ${data[item.day.icon]}`}></i>
                </p>
              </li>
            )
          })}
